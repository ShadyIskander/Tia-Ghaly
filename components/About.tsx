...
export default function About({ open, onClose }: Props) {
  ...
  }, [open, isMobile, campPhotos]);

  // Auto-rotate on desktop polaroid
  useEffect(() => {
    if (!open || isMobile || campPhotos.length === 0) return;
    const id = window.setInterval(() => {
      setCampIndex((prev) => (prev + 1) % campPhotos.length);
    }, 2600);
    return () => window.clearInterval(id);
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 998, backgroundColor: "#fdf6f0", overflowY: "auto", animation: "aboutFadeIn 0.3s ease" }}>
      <style>{`
        @keyframes aboutFadeIn { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
      `}</style>
      ...
      <button
        onClick={onClose}
        style={{
          position: "fixed", top: "1.3rem", left: "1.5rem",
          background: "none", border: "none", cursor: "pointer",
          fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.22em",
          textTransform: "uppercase", color: "var(--fg)",
          fontFamily: "var(--font-sans)", zIndex: 999,
          padding: "0.4rem 0", opacity: 0.7,
        }}
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: isMobile ? "5rem 1.25rem 4rem" : "7rem 3rem 6rem" }}>

        {/* ── Things Tia Loves + Photos ── */}
        <div style={{
          display: "grid",
          ...
          {/* Notebook list */}
          <div style={{
            background: "#faf9e8", border: "1px solid #e0ddb5",
            padding: "2rem 2rem 2rem 3.5rem", position: "relative",
            boxShadow: "3px 4px 18px rgba(0,0,0,0.10)", lineHeight: 1.8,
          }}>
            ...
            <h2 style={{ fontFamily: "var(--font-oswald), sans-serif", fontSize: "clamp(1.2rem, 2vw, 1.6rem)", fontWeight: 900, lineHeight: 1.1, marginBottom: "1.2rem", color: "#1a1a1a", position: "relative" }}>
              THINGS<br />TIA GHALY<br /><span style={{ color: "#f5846f" }}>LOVES:</span>
            </h2>
            <ol style={{ listStyle: "decimal", paddingLeft: "1.2rem", margin: 0, display: "flex", flexDirection: "column", gap: "0.15rem", position: "relative" }}>
              {["Babies and sunshine","Sat7 Kids show prep chaos","Calligraphy & lettering","Interactive packaging","Voice acting for animated films","Cairo street type","Oswald bold headlines","Anything technical or analogue","Cooking for loved ones","Unity in diversity"].map((item, i) => (
                <li key={i} style={{ fontSize: "0.88rem", color: "#2a2a2a", fontFamily: "var(--font-sans)" }}>{item}</li>
              ))}
            </ol>
          ...
          {/* Photos — slideshow on mobile, polaroids on desktop */}
          {isMobile ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
              {/* Polaroid 1 — rotating camp photo */}
              <div style={{ background: "#fff", padding: "0.9rem 0.9rem 3rem", boxShadow: "0 8px 26px rgba(0,0,0,0.13)", transform: "rotate(-1.5deg)" }}>
                <div style={{ width: "100%", aspectRatio: "4/3", background: "#efefef", overflow: "hidden" }}>
                ...
                {/* Dots */}
                <div style={{ display: "flex", justifyContent: "center", gap: "0.4rem", margin: "0.5rem 0 0.3rem" }}>
                  {campPhotos.map((_, i) => (
                    <button key={i} onClick={() => setCampIndex(i)} style={{
                      width: "5px", height: "5px", borderRadius: "50%", border: "none", padding: 0, cursor: "pointer",
                      background: i === campIndex ? "#0a0a0a" : "#ababab", opacity: i === campIndex ? 1 : 0.4,
                    }} />
                  ))}
                </div>
              ...
              {/* Polaroid 2 — video */}
              <div style={{ background: "#fff", padding: "0.9rem 0.9rem 3rem", boxShadow: "0 8px 26px rgba(0,0,0,0.12)", transform: "rotate(1.5deg)" }}>
                <div style={{ width: "100%", aspectRatio: "4/3", background: "#dfe7e4", overflow: "hidden" }}>
                  <video
                    key={currentVideo} src={currentVideo}
              ...
              {/* Polaroid 3 — on set */}
              <div style={{ background: "#fff", padding: "0.9rem 0.9rem 3rem", boxShadow: "0 8px 26px rgba(0,0,0,0.12)", transform: "rotate(-1deg)" }}>
        ...
        <div style={{
          ...
          {/* Column 1 */}
          <div>
            <h2 style={{ fontFamily: "var(--font-oswald), sans-serif", fontSize: "1.5rem", fontWeight: 700, marginBottom: "1.5rem", color: "var(--fg)" }}>Education</h2>
            ...
            <h2 style={{ fontFamily: "var(--font-oswald), sans-serif", fontSize: "1.5rem", fontWeight: 700, marginBottom: "1.5rem", color: "var(--fg)" }}>Exhibitions</h2>
            <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "2.5rem" }}>
              {[
            ...
            <h2 style={{ fontFamily: "var(--font-oswald), sans-serif", fontSize: "1.5rem", fontWeight: 700, marginBottom: "1.5rem", color: "var(--fg)" }}>Awards</h2>
          ...
          {/* Column 2 */}
          <div>
            <h2 style={{ fontFamily: "var(--font-oswald), sans-serif", fontSize: "1.5rem", fontWeight: 700, marginBottom: "1.5rem", color: "var(--fg)" }}>Experience</h2>
              ...
              <div key={key} style={{ borderBottom: "1px solid var(--border)" }}>
                <button
                  ...
                  style={{
                    ...
                    fontFamily: "var(--font-oswald), sans-serif", fontSize: "1rem", fontWeight: 500,
                    color: "var(--fg)", textAlign: "left",
                  }}
                >
                  <span>{label}</span>
                  <span style={{ fontSize: "1.2rem", lineHeight: 1, color: accentColor }}>{expanded === key ? "−" : "+"}</span>
                </button>
                {expanded === key && (
                  <p style={{ fontSize: "0.865rem", color: "var(--fg)", lineHeight: 1.8, paddingBottom: "1.1rem", whiteSpace: "pre-line", opacity: 0.85 }}>
                    {BIOS[key as keyof typeof BIOS]}
                  </p>
                )}
              </div>
            ))}
            <h2 style={{ fontFamily: "var(--font-oswald), sans-serif", fontSize: "1.5rem", fontWeight: 700, marginTop: "2.5rem", marginBottom: "1.5rem", color: "var(--fg)" }}>Get in Touch</h2>
            <ul style={{ listStyle: "none", padding: 0, display: "flex", flexDirection: "column", gap: "0.55rem" }}>
              {[
                ...
                <li key={i} style={{ fontSize: "0.865rem", lineHeight: 1.65 }}>
                  {item.href ? (
                    <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" style={{ color: "var(--fg)", fontWeight: 600, textDecoration: "underline" }}>
                      {item.value}
                    </a>
                  ) : (
